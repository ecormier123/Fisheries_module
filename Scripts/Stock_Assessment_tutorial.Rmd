...
2. How well does the Stock Assessment compare to the 'actual biomass'
...
ii.   On line 89 of this file you can change the number of tows used in the survey.  Default is 20 tows
iii.  On lines 95-97 of this file you can change the underlying biomass distribution
      - To do this you add a # symbol at the start of the lines you do not want, and removed it from the line you do want
iv.   You can change the number of simulations run on line 100.
...
```{r q-m-priors-posts,echo=F,message=F,warning=F,fig.cap = "The Prior and Posterior distributions for catchability and mortality.  The Prior distribution is approximated by the line, while the model realizations (aka Posteriors) are given by the histogram (bars)"}
...
plt.q <- ggplot(q.post) + geom_histogram(aes(Posterior)) + geom_freqpoly(aes(Prior)) + xlab("Catchability") + ylab("") + theme_bw()
plt.m <- ggplot(m.post) + geom_histogram(aes(Posterior)) + geom_freqpoly(aes(Prior)) + xlab("Natural Mortality") + ylab("") + theme_bw()
