...
# Play around with the q and m priors to see how these influence your results..
# They are both set up as beta distribution priors, so these plots help you to understand what that is.
...
direct <- getwd()
source(paste(direct,"/Scripts/Functions/Model_function.R",sep=""))

# Run the model, see the function for an explanation of all the model options.
run_model(mod.dat = mod.dat, direct = direct,
          jags.model = "models/delay_difference_model_base.bug",
          nchains=6,niter = niter,nburn = 0.25*n.iter,
          m.prior = m.prior,q.prior = q.prior
          )


load(file=paste(direct,"/Results/Lecture_3/Model_results.RData",sep=""))
